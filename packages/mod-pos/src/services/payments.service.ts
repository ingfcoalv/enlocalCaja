...
async function ensurePaymentsTable(db: any): Promise<void> {
  ...
  await db.execute(sql`
    DO $$ BEGIN
      ...
      ALTER TABLE payments ADD COLUMN IF NOT EXISTS register_id uuid;
...
export async function createPayment(
  ...
  userId: string,
  shiftId: string | null
): Promise<{ payments: PaymentRow[]; invoice: any }> {
  ...
  const insertedPayments: PaymentRow[] = []

  for (const payment of data.payments) {
    const result = await db.execute(
      sql`INSERT INTO payments (invoice_id, method, amount, reference, tip, shift_id, user_id, created_at)
  ...
  const paidResult = await db.execute(
    sql`SELECT coalesce(sum(amount), 0)::numeric as total_paid
        FROM payments
...
export async function refundPayment(
  ...
  reason: string,
  userId: string
): Promise<{ refund: PaymentRow; invoice: any }> {
  await ensurePaymentsTable(db)
