...
router.post('/:orderId/print', requirePermission('pos.read'), async (req, res) => {
    try {
...
router.post('/:orderId/reprint', requirePermission('pos.read'), async (req, res) => {
    try {
        const db = req.app.get('db');
        const { orderId } = req.params;
        const receiptText = await generateReceipt(db, orderId);
...
router.get('/:orderId/preview', requirePermission('pos.read'), async (req, res) => {
    try {
        const db = req.app.get('db');
        const { orderId } = req.params;
