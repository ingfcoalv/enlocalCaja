...
export async function listSales(db, filters) {
    ...
    if (filters.q) {
        conditions.push(ilike(invoices.series, `%${filters.q}%`));
    }
...
export async function getSaleDetail(db, pool, saleId) {
    // Fetch invoice
    ...
    const paymentsResult = await pool.query(`SELECT id, method, amount::numeric, reference, tip::numeric, is_abono, customer_id, created_at
     FROM payments
     WHERE invoice_id = $1
    ...
    const linksResult = await pool.query(`SELECT il.id, il.cfdi_invoice_id, i.series, i.folio, i.status, i.type, i.uuid_fiscal, i.stamped_at, i.total::numeric
     FROM invoice_links il
     JOIN invoices i ON il.cfdi_invoice_id = i.id
     WHERE il.sale_invoice_id = $1
     ORDER BY i.created_at DESC`, [saleId]);
    const linkedInvoices = (linksResult.rows || []).map((r) => ({
    ...
    return {
        ...
        items,
        payments,
        abonos: payments.filter((p) => p.isAbono),
        totalPaid,
        balance: Math.max(0, balance),
        linkedInvoices,
    };
}
export async function getSalesDashboard(db, date) {
    const dayStart = new Date(date + 'T00:00:00');
    const dayEnd = new Date(date + 'T23:59:59.999');
    const dateCondition = and(gte(invoices.createdAt, dayStart), lte(invoices.createdAt, dayEnd), eq(invoices.source, 'pos'));
    // Total sales and orders
    const totalsResult = await db
        .select({
        totalSales: sql `coalesce(sum(${invoices.total}), 0)::numeric`,
    ...
    let byMethod = [];
    try {
        const byMethodResult = await db.execute(sql `SELECT p.method, coalesce(sum(p.amount), 0)::numeric as total, count(*)::int as count
          FROM payments p
          JOIN invoices i ON p.invoice_id = i.id
          WHERE i.created_at >= ${dayStart} AND i.created_at <= ${dayEnd} AND i.source = 'pos'
          GROUP BY p.method
          ORDER BY total DESC`);
        const rows = byMethodResult.rows ?? byMethodResult;
        byMethod = Array.isArray(rows) ? rows.map((r) => ({
            method: r.method,
            total: parseFloat(r.total),
    ...
    const byHourResult = await db
        .select({
        hour: sql `extract(hour from ${invoices.createdAt})::int`,
        total: sql `coalesce(sum(${invoices.total}), 0)::numeric`,
        count: sql `count(*)::int`,
    })
        .from(invoices)
        .where(dateCondition)
    ...
    const byHour = byHourResult.map((r) => ({
        hour: r.hour,
        total: parseFloat(r.total),
        count: r.count,
    }));
    ...
    const topProductsResult = await db
        .select({
        name: products.name,
        quantity: sql `coalesce(sum(${invoiceItems.quantity}), 0)::numeric`,
        total: sql `coalesce(sum(${invoiceItems.amount}), 0)::numeric`,
    })
        .from(invoiceItems)
...
export async function getSalesSummary(db, from, to) {
    const dateFrom = new Date(from + 'T00:00:00');
    const dateTo = new Date(to + 'T23:59:59.999');
    const dateCondition = and(gte(invoices.createdAt, dateFrom), lte(invoices.createdAt, dateTo), eq(invoices.source, 'pos'));
    const result = await db
        .select({
        totalSales: sql `coalesce(sum(${invoices.total}), 0)::numeric`,
        totalOrders: sql `count(*)::int`,
        totalTax: sql `coalesce(sum(${invoices.tax}), 0)::numeric`,
    })
        .from(invoices)
        .where(dateCondition);
    ...
    const discountResult = await db
        .select({
        totalDiscount: sql `coalesce(sum(${invoiceItems.discount}), 0)::numeric`,
    })
        .from(invoiceItems)
        .innerJoin(invoices, eq(invoiceItems.invoiceId, invoices.id))
        .where(dateCondition);
