...
export async function listCreditCustomers(db, pool, filters) {
    const page = filters.page || 1;
    const limit = filters.limit || 50;
    const offset = (page - 1) * limit;
    const search = filters.q ? `%${filters.q}%` : null;
    const result = await pool.query(`WITH credit_invoices AS (
      SELECT
        i.customer_id,
        i.id as invoice_id,
        ...
        coalesce((
          SELECT sum(p.amount)
          FROM payments p
          WHERE p.invoice_id = i.id AND p.amount > 0 AND p.method != 'credit'
        ), 0) as total_paid
      FROM invoices i
      WHERE i.source = 'pos'
    ...
    LIMIT $1 OFFSET $2`, search ? [limit, offset, search] : [limit, offset]);
    const countResult = await pool.query(`WITH credit_invoices AS (
      SELECT i.customer_id, i.total::numeric as invoice_total,
        coalesce((SELECT sum(p.amount) FROM payments p WHERE p.invoice_id = i.id AND p.amount > 0 AND p.method != 'credit'), 0) as total_paid
      FROM invoices i
    ...
    JOIN customers c ON cb.customer_id = c.id
    ${search ? 'WHERE c.name ILIKE $1' : ''}`, search ? [search] : []);
...
export async function getCustomerCreditDetail(db, pool, customerId) {
    // Fetch customer
    ...
    const creditInvoices = invoicesResult.rows.map((r) => {
...
export async function createAbono(db, pool, data) {
    ...
    const paidResult = await pool.query(`SELECT coalesce(sum(amount), 0)::numeric as total_paid
     FROM payments
     WHERE invoice_id = $1 AND amount > 0 AND method != 'credit'`, [invoiceId]);
    ...
    for (const payment of payments) {
        const result = await pool.query(`INSERT INTO payments (invoice_id, method, amount, reference, shift_id, user_id, customer_id, is_abono)
       VALUES ($1, $2, $3, $4, $5, $6, $7, true)
       RETURNING *`, [invoiceId, payment.method, payment.amount, payment.reference || null, shiftId || null, userId, customerId]);
        insertedPayments.push(result.rows[0]);
    }
    // Recalculate balance
    const newPaidResult = await pool.query(`SELECT coalesce(sum(amount), 0)::numeric as total_paid
     FROM payments
     WHERE invoice_id = $1 AND amount > 0 AND method != 'credit'`, [invoiceId]);
